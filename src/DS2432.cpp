...
DS2432::DS2432(uint8_t ID1, uint8_t ID2, uint8_t ID3, uint8_t ID4, uint8_t ID5, uint8_t ID6,
               uint8_t ID7)
...
void DS2432::duty(OneWireHub *const hub)
{
...
void DS2432::clearMemory(void) { memset(memory, static_cast<uint8_t>(0x00), sizeof(memory)); }

void DS2432::clearScratchpad(void)
{
    memset(scratchpad, static_cast<uint8_t>(0x00), SCRATCHPAD_SIZE);
}

bool DS2432::writeMemory(const uint8_t *const source, const uint8_t length, const uint8_t position)
{
...
bool DS2432::writeScratchpad(const uint8_t *const source)
{
    memcpy(scratchpad, source, SCRATCHPAD_SIZE);
    return true;
}

bool DS2432::readMemory(uint8_t *const destination, const uint16_t length,
...
void DS2432::setPageProtection(const uint8_t position)
{
...
bool DS2432::getPageProtection(const uint8_t position) const
{
...
void DS2432::setPageEpromMode(const uint8_t position)
{
...
bool DS2432::getPageEpromMode(const uint8_t position) const
{
    if (position < 1 * PAGE_SIZE)
    {
...
bool DS2432::updatePageStatus(void)
{
    page_eprom_mode = 0;
    page_protection = 0;
